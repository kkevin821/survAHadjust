...
Developers who want to work with the source code can clone the
repository, open the package folder in RStudio, and install the local
version with:
...
output with `adjusted_ahsw()` as long as it is formatted as an adjusted
...
The helper function `ahsw_table()` uses the confidence interval level in
the column names. For example, with `conf_level = 0.90`, the formatted
table uses column names such as:
